...
parser.add_argument('--exp', type=str,
                    default='cutmix', help='Name of Experiment')
...
def train(args,snapshot_path,exp_path):
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(zip(trainloader_s,trainloader_t)):
            ...
            # Supervised loss
            outputs = model(volume_batch_s)['seg']
            outputs_soft = torch.softmax(outputs, dim=1)
            loss_dice = dice_loss(outputs_soft, label_batch_s)
            ...
            cut_mask = mask_generator.generate_params(int(batch_size_half / 2),
                                                      (volume_batch_t1.shape[-3],
                                                       volume_batch_t1.shape[-2],
                                                       volume_batch_t1.shape[-1]))
            cut_mask = torch.tensor(cut_mask).type(torch.FloatTensor).cuda()
            batch_tx_mixed = volume_batch_t0 * (1.0 - cut_mask) + \
                             volume_batch_t1 * cut_mask
            outputs_tx = model(batch_tx_mixed)['seg']
            outputs_soft_tx = torch.softmax(outputs_tx, dim=1)
            with torch.no_grad():
                ...
                batch_pred_mixed = ema_output_t0 * (1.0 - cut_mask) + \
                                   ema_output_t1 * cut_mask
            if args.consistency_type == 'mse':
                consistency_loss = mse_loss(outputs_soft_tx, batch_pred_mixed)
            elif args.consistency_type == 'kv':
                consistency_loss = F.kl_div(torch.log(outputs_soft_tx), batch_pred_mixed, reduction='mean')
            consistency_weight = get_current_consistency_weight(epoch_num)

            # Total loss
            loss = loss_dice + consistency_weight * consistency_loss
            ...
            iter_num = iter_num + 1
            logging.info(
                'iteration %d : loss : %f loss_dice : %f loss_con : %f' %
                (iter_num, loss.item(), loss_dice.item(), consistency_loss.item()))
