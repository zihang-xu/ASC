...
parser.add_argument('--exp', type=str,
                    default='fda', help='Name of Experiment')
...
def train(args,snapshot_path,exp_path):
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(zip(trainloader_s,trainloader_t)):
            ...
            # FFT
            src_in_trg = FDA_source_to_target(src_img=volume_batch_s,trg_img=volume_batch_t,L=0.09)
            outputs_soft_st = torch.softmax(model(src_in_trg)['seg'], dim=1)
            loss_dice += dice_loss(outputs_soft_st, label_batch_s)
            en_loss = entropy_loss(torch.softmax(model(volume_batch_t)['seg'], dim=1),C=8)
            consistency_weight = get_current_consistency_weight(epoch_num)
            # Total loss
            loss = loss_dice + en_loss * consistency_weight
            ...
            iter_num = iter_num + 1
            logging.info(
                'iteration %d : loss : %f loss_dice : %f loss_en : %f' %
                (iter_num, loss.item(), loss_dice.item(), en_loss.item()))
