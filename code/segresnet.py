...
class SegResNet(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        down_x.reverse()
        encoder = x
        segmentation, feature = self.decode(x, down_x)
        return {'seg': segmentation, 'feat': feature, 'enco': encoder}
...
class SegResNetVAE(SegResNet):
    ...
    def _get_vae_loss(self, net_input: torch.Tensor, vae_input: torch.Tensor):
        ...
        return vae_loss,feature_domain
    ...
    def forward(self, x):
        ...
        vae_loss, feature_domain = self._get_vae_loss(net_input, vae_input)
        return {'seg': segmentation, 'vae_loss': vae_loss, 'feat': feature_domain}


class SegResNet1(nn.Module):
    """
    SegResNet based on `3D MRI brain tumor segmentation using autoencoder regularization
    <https://arxiv.org/pdf/1810.11654.pdf>`_.
    The module does not include the variational autoencoder (VAE).
    ...
    #     )
    #
    #     for class_c in range(out_channels):
    ...
    #
    # def forward_proj(self, features):
    #     return self.proj(features)
    #
    # def forward_pred(self, features):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        x1, down_x = self.encode(x)
        down_x.reverse()

        encoder = x1
        segmentation,feature = self.decode(x1, down_x)
        projection = self.proj(feature)
        projection = F.normalize(projection, dim=1)
        return {'seg': segmentation, 'feat': feature, 'proj': projection, 'enco': encoder}


class SegResNet2(nn.Module):
    """
    SegResNet based on `3D MRI brain tumor segmentation using autoencoder regularization
    <https://arxiv.org/pdf/1810.11654.pdf>`_.
    The module does not include the variational autoencoder (VAE).
    ...
    def __init__(
        ...
        self.up_layers, self.up_samples = self._make_up_layers()
        self.conv_final = self._make_final_conv(out_channels)
        self.dir_final = self._make_final_conv(out_channels)
        self.proj = nn.Sequential(
            nn.Conv3d(32, 32, kernel_size=1),
            nn.BatchNorm3d(32),
            nn.ReLU(),
...
class SegResNet3(nn.Module):
    ...
    def forward(self, x: torch.Tensor, output=None) -> torch.Tensor:
        x1, down_x = self.encode(x)
        down_x.reverse()

        segmentation,feature = self.decode(x1, down_x)
        sample = {'seg': segmentation, 'feat': feature}
        if output == 'pred' or output == 'proj':
            projection = self.proj(feature)
            projection = F.normalize(projection, dim=1)
            sample['proj'] = projection
            if output == 'pred':
                prediction = self.pred(projection)
                prediction = F.normalize(prediction, dim=1)
                sample['pred'] = prediction
        return sample


class SegResNet4(nn.Module):
    """
    SegResNet based on `3D MRI brain tumor segmentation using autoencoder regularization
    <https://arxiv.org/pdf/1810.11654.pdf>`_.
    The module does not include the variational autoencoder (VAE).
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        encoder = x1
        encoder_class = self.avg(encoder)
        encoder_class = encoder_class.view(encoder_class.size(0),-1)
        encoder_class = self.fcn(encoder_class)
        encoder_class = encoder_class.sigmoid()
        segmentation,feature = self.decode(x1, down_x)
        projection = self.proj(feature)
        projection = F.normalize(projection, dim=1)
        return {'seg': segmentation, 'feat': feature, 'proj': projection, 'enco': encoder_class}


class SegResNet5(nn.Module):
    """
    SegResNet based on `3D MRI brain tumor segmentation using autoencoder regularization
    <https://arxiv.org/pdf/1810.11654.pdf>`_.
    The module does not include the variational autoencoder (VAE).
...
class SegResNet6(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        down_x.reverse()
        enco = x
        enco = enco.view(enco.size(0), -1)
        encoder = self.head(enco)
        segmentation, feature = self.decode(x, down_x)
        return {'seg': segmentation, 'feat': feature, 'enco': encoder}


class SegResNet7(nn.Module):
    """
    SegResNet based on `3D MRI brain tumor segmentation using autoencoder regularization
    <https://arxiv.org/pdf/1810.11654.pdf>`_.
    The module does not include the variational autoencoder (VAE).
    ...
    def __init__(
        ...
        self.up_layers, self.up_samples = self._make_up_layers()
        self.conv_final = self._make_final_conv(out_channels)
        self.conv_final2 = self._make_final_conv(out_channels)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        down_x.reverse()
        encoder = x
        segmentation, sdm = self.decode(x, down_x)

        return {'seg': segmentation, 'sdm': sdm}


class SegResNet8(nn.Module):
    """
    SegResNet based on `3D MRI brain tumor segmentation using autoencoder regularization
    <https://arxiv.org/pdf/1810.11654.pdf>`_.
    The module does not include the variational autoencoder (VAE).
    ...
    def __init__(
        ...
        self.up_layers, self.up_samples = self._make_up_layers()
        self.conv_final = self._make_final_conv(out_channels)
        self.conv_final2 = self._make_final_conv(n_rays)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        down_x.reverse()
        # encoder = x
        segmentation, dist = self.decode(x, down_x)

        return {'seg': segmentation, 'dist': dist}


class SegResNet9(nn.Module):
    """
    SegResNet based on `3D MRI brain tumor segmentation using autoencoder regularization
    <https://arxiv.org/pdf/1810.11654.pdf>`_.
    The module does not include the variational autoencoder (VAE).
...
class conv_block(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.conv_block = nn.Sequential(nn.Conv3d(in_channels, in_channels, 1, stride=1),
            nn.BatchNorm3d(in_channels),
            nn.ReLU(True),
            nn.Conv3d(in_channels, out_channels, 1, stride=1),
            nn.BatchNorm3d(out_channels),
            nn.ReLU(True))
        self.mlp = nn.Sequential(nn.Linear(in_features=12*12*12, out_features=512),
                                 nn.ReLU(),
